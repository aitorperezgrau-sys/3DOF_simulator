...
class flight_3dof:
    ...
    def __init__(
        ...
        self.check_input_parameters(
            rocket, atm, initial_altitude, inclination, heading, impact_elevation
        )
        self.rocket = rocket
        self.atm = atm
    ...
    def check_input_parameters(
        ...
        if isinstance(initial_altitude, (float, int)):
            if initial_altitude < 0:
                raise ValueError(
                    "The initial altitude above the sea level must be greater than 0"
                )
            elif initial_altitude > 81020 - 1e-6:
                raise ValueError(
                    "The initial altitude cannot be greater or equal than 81020 m above sea level"
                )
        else:
            raise ValueError(
                "The initial altitude above the sea level must be a float or int"
            )
        ...
        if isinstance(impact_elevation, (float, int)):
            if impact_elevation < 0:
                raise ValueError(
                    "The impact altitude above the sea level must be greater than 0"
                )
            elif impact_elevation > 81020 - 1e-6:
                raise ValueError(
                    "The impact altitude cannot be greater or equal than 81020 m above sea level"
                )
        else:
            raise ValueError(
                "The impact altitude above the sea level must be a float or int"
            )
    ...
    def simulate(self):
        ...
        while self.solver.successful() and not finish:
            ...
            if (
                self.solver.y[2] + self.initial_altitude <= self.impact_elevation
                and self.solver.y[5] < 0
                and self.solver.t > 0.5
            ):
                finish = True
        last_u = self.us[-1]
